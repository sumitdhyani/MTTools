...
There is nothing to build. Just clone with --rescursive flag as the whole library is header-only if you don't want to build/run Unit Tests.

## To build amn run unit tests, following are the prerequisites:
- CMake(3.5.0+)
- GTest, environment variable **GTEST_ROOT** should be set to the googletest source/install directory root
- On windows clang with C++20 support
- On linux g++ with C++20 support
  
If all the prerequisites are met then just go to the root directory and run build.sh/build.bat for linux/windows and you're ready to go.
