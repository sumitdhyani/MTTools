...
namespace mtInternalUtils
{
  ...
  private:
    ...
    std::map<time_point, T, std::less_equal<time_point>> m_processingQueue;
      ...
      {
        {
          ...
          for (auto const &timeItemPair : local)
          {
            m_processingQueue.insert(timeItemPair);
          }
        }
